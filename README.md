...
Knowledge Copilot and Knowledge Audit screenshots can be added here when available.
...
The old static app files under `static/` are legacy/unrouted and are not the active application. They remain in the repository for now but should not be treated as the current UI.
...
├── static/             # Legacy static app files; not active routes
...
5. Leave `PORT` unset — Render injects it automatically. The Dockerfile reads `${PORT:-8000}`.
   The production command is `uvicorn api:app --host 0.0.0.0 --port ${PORT:-8000}` and does not use `--reload`.
